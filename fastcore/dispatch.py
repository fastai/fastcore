...
__all__ = ['lenient_issubclass', 'sorted_topologically', 'TypeDispatch', 'DispatchReg', 'typedispatch', 'cast',
           'retain_meta', 'default_set_meta', 'retain_type', 'retain_types', 'explode_types', 'with_cast', 'casted']
...
# Cell
def casted(f):
    "Decorator that cast output of func"
    out_type = anno_ret(f)
    @functools.wraps(f)
    def _inner(*args, **kwargs):
        args = list(args)
        return cast(f(*args, **kwargs), out_type)
    return _inner