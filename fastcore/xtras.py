...
__all__ = ['dict2obj', 'obj2dict', 'repr_dict', 'is_listy', 'shufflish', 'mapped', 'IterLen', 'ReindexCollection',
           'maybe_open', 'image_size', 'bunzip', 'join_path_file', 'loads', 'loads_multi', 'untar_dir', 'file_extract',
...
def file_extract(fname, dest=None):
    "Extract `fname` to `dest` using `tarfile` or `zipfile` based on file type."
    if dest is None: dest = Path(fname).parent
    fname = str(fname)
    if   fname.endswith('gz'):  tarfile.open(fname, 'r:gz').extractall(dest)
    elif fname.endswith('bz2'): tarfile.open(fname, 'r:bz2').extractall(dest)
